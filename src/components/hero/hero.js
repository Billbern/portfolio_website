...
export default class HeroSection extends Component {
    ...
    constructor() {
        ...
        this.state = {
            animation: [
                "danceAround25 6s linear 0.5ms infinite",
            "danceAround25 6s linear 10ms reverse infinite",
            "danceAround50 6s linear 0.5ms infinite",
            "danceAround50 6s linear 10ms reverse infinite"
            ],
            coord: null
        }
    }
                                ...
                                <a href="/">
                                    <svg viewBox="0 0 576 576" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M469.261 306.004a189.552 188.053 0 00-178.927 187.757 189.552 188.053 0 009.884 59.932 189.552 188.053 0 00178.927-187.757 189.552 188.053 0 00-9.884-59.932z"
                                            fillOpacity=".547" fill="#fff" paintOrder="stroke fill markers" />
                        ...
                        <div className="hero-intro">
                            <div className="icons backend-icons">
                                <div className="icon-container">
                                    {
                                        [{ title: 'nodejs', file: 'nodejs.svg' }, { title: 'express', file: 'express.svg' }, { title: 'python', file: 'python.svg' }, { title: 'flask', file: 'flask.svg' }, { title: 'docker', file: 'docker.svg' }, { title: 'postgreSQL', file: 'postgresql.svg' }, { title: 'mongoDB', file: 'mongodb.svg' }].map((item, key) => {
                                            return (
                                                <div className="tech-icons" key={key} style={{ top: `${this.state.coord[key + 1][0]}%`, left: `${this.state.coord[key + 1][1]}%`, animation: `${ (key+1)%2 === 0 ? (key+1) < 5 ? this.state.animation[0] : this.state.animation[1] : (key+1) < 5 ? this.state.animation[2] : this.state.animation[3] }` }}>
                            ...
                            <div className="intro">
                                ...
                                <div className="content">
                                    ...
                                    <div className="content-buttons">
                                        <button className="business">email</button>
                            ...
                            <div className="icons frontend-icons">
                                <div className="icon-container">
                                    {
                                        [{ title: 'html5', file: 'html5.svg' }, { title: 'css3', file: 'css3.svg' }, { title: 'sass', file: 'sass.svg' }, { title: 'tailwindcss', file: 'tailwindcss.svg' }, { title: 'react', file: 'react.svg' }, { title: 'javascript', file: 'javascript.svg' }, { title: 'redux', file: 'redux.svg' }].map((item, key) => {
                                            return (
                                                <div className="tech-icons" key={key} style={{ top: `${this.state.coord[key + 1][0]}%`, left: `${this.state.coord[key + 1][1]}%`, animation: `${ (key+1)%2 === 0 ? (key+1) < 5 ? this.state.animation[0] : this.state.animation[1] : (key+1) < 5 ? this.state.animation[2] : this.state.animation[3] }` }}>
                                                    <img src={require(`../../assets/img/icons/${item.file}`).default} alt={item.title} />
                                                </div>
